<template>
	<div id="tabs-api-ex1" class="tab-pane m-0 p-0">
		<div class="row row-sm align-items-center">
			<div class="col">
				<p class="label-text mb-2 text-capitalize" style="color: #5c5c5c; font-size: 14px; line-height: 19px">
			...
			<div class="col">
				...
				<p class="text-primary font-weight-bold mb-2 h-4" />
			</div>
		...
		<div class="row row-sm align-items-center">
			<div class="col">
				<p class="label-text mb-2 text-capitalize" style="color: #5c5c5c; font-size: 14px; line-height: 19px">
					{{ merchantDetails.environment }} Webhook URL
				</p>
				...
				<h4 v-if="!isEdited" class="card-title m-0 font-weight-bold align-items-center">
					...
					<span class="icon ml-1" @click="copyToClipboard(webhook.url)">
						<svg
							width="13"
							height="13"
							viewBox="0 0 13 13"
				...
				<h4 v-else class="card-title m-0 font-weight-bold align-items-center">
					<input
						...
						:placeholder="
							getMerchantDetails.is_live
								? 'https://www.klump.com/merchants/live'
								: 'https://www.klump.com/merchants/test'
						"
					/>
				</h4>
			</div>
			<div class="col-auto">
				<button
					...
					@click="handleCreateWebhook(getMerchantDetails.is_live)"
				>
					<span class="mb-1 mr-1">
					...
					<span v-if="getStatus === 'loading'" class="px-4">
						<Loader />
					</span>
					<span v-else>
...
export default {
	...
	computed: {
		...mapGetters(['getLoggedUser', 'getStatus', 'getMerchantDetails']),
		webhook() {
			return this.getMerchantDetails.is_live ? this.liveWebhook : this.testWebhook;
		}
	},
	watch: {
		getStatus(val) {
